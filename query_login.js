...
async function main(argv) {
  ...
  try {
    ...
    const response = await contract.evaluateTransaction('login', myArgs[0].toString(), myArgs[1].toString());

    // process response
    console.log('Process buy transaction response.');
    var str = '[{ "value" : "erro - user null"}]'
    if(response.value == "erro - user nulo") console.log(JSON.parse(str));
